...
Tested to use both l1 and l2 regulization. 
```{python}
...
Set our threshold to 0.3 to help with class imbalance.
```{python}
...
The modelling portion of our project was done using logistic regression. We encoded a binary
response variable indicating whether or not the sample was compliant with national standards
for drinking water. Since the resulting variable was so imbalanced, we used SMOTE to oversample
the underrepresented class. Before fitting the logistic regression model, we used grid search
to obtain the best parameters. We fit the model with those parameters and evaluated by getting
predicted probabilities and converting to binary once again based off the probabilities being 
greater than versus less than $0.5$. Furthermore, we created a confusion matrix and printed the
accuracy score, recall, precision, and F1 score. The recall was incredibly high, around $0.99$,
and the accuracy was $0.75$, which suggests the model performed fairly. 
