...
model_data.drop(columns = ['borough_Bronx', 'zip code', 'latitude', 'longitude'], 
                                                                  inplace = True)
...
feature_names = model_data.drop(columns = ['compliant']).columns

# contribution of each feature to each principal component
contributions = pd.DataFrame(components, columns = feature_names, 
                              index=[f'PC{i+1}' for i in range(components.shape[0])])

# get feature importances and sort
feature_contributions = importances.dot(components)
feature_importance_df = pd.DataFrame(feature_contributions, index = feature_names, 
                                                            columns = ['Importance'])
